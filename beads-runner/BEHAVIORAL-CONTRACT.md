...
> exists today (**2477 lines**, refreshed against the script at this commit). It
...
> since ~tripled to 2477 lines across ~40 commits (idle-on-empty-queue,
...
> `post_close_audit`, Node-v25 PATH prime, …). This revision re-verifies every
> prior entry against current line numbers and adds **BC-43…BC-65** for behavior
> introduced since. All prior BC-IDs are preserved for referential stability.
>
> Classification key:
...
**Source:** lines 1–3 (header intent); 1769–1778 (per-task `claude -p` spawn inside the main loop).
...
**Source:** 599–605 (startup snapshot), 608–630 (`next_task` drains orphans before `bd ready`).
...
**Source:** 603–605. `read -ra` on empty input yields a 1-element array `("")`; line 605 detects exactly that shape (`${#ORPHANED_IDS[@]} -eq 1 && -z "${ORPHANED_IDS[0]}"`) and resets to an empty array.
...
**Source:** 608–630 (esp. 616–627 status branch, 618–619 tail-preservation, 631–643 fall-through to epic-filtered `bd ready`).
...
**Source:** 1388–1450 (idle branch); legacy exit gate 1403–1408 and 1446; `IDLE_NOTIFIED` 1417/1420/1448; startup `.stop-beads` rm 296.
...
**Source:** 731–774 (`select_workable_task`); idle two-tier poll 1410–1447.
...
**Source:** 631–643 (query-layer filter), 686–701 (validate_task epic skip + dual-shape parse).
...
**Source:** 678–684 (blocked re-check); cost-free skip via `select_workable_task` (754–774) + idle/continue (1388–1450).
...
**Source:** config default 68–86; gate 651–676 (runs ahead of deps/epic/parent).
...
**Source:** config default 87–105; check at the tail of `validate_task` (after the parent/epic gate, before `return 0`). Design: UX-DESIGN-V2.md §8.5 / design/cross-ws.md §2.4. Conformance: `conformance/assertions/bc-xws-scope-check.sh`.
...
**Source:** 785–795 (exit-0 branch; fail-open stderr echo at 790); consumed at 2159.
...
*(Stale-comment note, not a behavior: a comment at line 1156 inside `post_close_audit` still cites "line 743" for this fail-open; the actual echo is now line 790.)*
...
**Source:** 798–818 (ordered greps in `classify_failure`); marker emission in the parser at 1818–1920 (esp. 1837–1841 api_retry, 1868–1885 result).
...
**Source:** 805–808 (ordering in classifier), 1882–1885 (parser pattern-match + `is_error` guard).
...
**Source:** 1841 (api_retry path), 1874 (result-stop-reason path), 809–812 (classifier collapse).
...
**Source:** 1501–1518 (top-of-loop `MAX_RETRIES` gate), 2333–2342 (RATE_LIMIT), 2344–2370 (overflow/max-tokens skip), 2372–2389 (task-not-closed retry-once), 2391–2403 (generic distinct-task breaker increment).
...
**Source:** 2391–2403 (distinct-task increment), 2288 (`SUCCESS` resets), 2419–2433 (breaker + rationale string), config default line 62.
...
**Source:** 2300, 2318, 2336, 2347, 2359, 2375, 2394 (uniform `--status=open`); STUCK bypass 2281–2283.
...
**Source:** 1501–1518 (`LAST_FAILED_ID` equality gate), 599–605 (orphan snapshot is startup-only).
...
|------|-----------|--------|
| **0** | Normal end: queue drained with `RUNNER_EXIT_ON_DRAIN=1`, **or** graceful stop file consumed (`break`/`break 3`). Falls off the end of the script after the `while` loop. | 1317/1330/1403–1408/1446 (`break`), 2451–2477 (tail; no literal `exit 0`) |
| **1** | `SIGINT`/`SIGTERM` received → `cleanup()` trap. | 408, 378–406 |
| **2** | Consecutive-failure circuit breaker tripped. | 2419–2432 |
| **3** | `AUTH_FAILURE` (terminal). | 2296–2312 |
| **4** | `BILLING_ERROR` (terminal). | 2314–2330 |
...
**Correction (the original "no EXIT trap" finding is now WRONG):** There **is** a `trap _final_subshell_reap EXIT` (line 414) in addition to `trap cleanup INT TERM` (line 408). `_final_subshell_reap` (the subshell PG-reap, BC-44) runs on **every** exit path — clean drain, exit 2/3/4, after `cleanup`'s `exit 1`, *and* on an unguarded `set -e` abort. So the leaked-subshell hazard the original entry flagged is **closed**. What remains asymmetric is `runner_cleanup` (the project hook), not the EXIT trap — see BC-36. `WORKER_STUCK_EXIT=7` (line 87) is the **worker's** deliberate-stuck sentinel, never an exit code the *runner* process returns.
...
**Source:** 1943–2034 (loop; threshold 2003, soft warning 2029, snapshot 2011–2017, staged kill 2022–2027).
...
**Source:** 1993–2002 (`EFFECTIVE_TIMEOUT` math); config 88/96; parser maintenance of `TASK_INFLIGHT_FILE` (BC-60).
...
**Source:** parser stamp 1801–1817; watchdog branch 1947–1973; config 106.
...
**Source:** `ACTIVITY_FILE` stamp 1788/1800; `TASK_INFLIGHT_FILE` 1787/1843–1863; epoch-floor guards 1958/1974–1985/2074.
...
**Source:** 901–907 (label guard), 925–929 (create), 941 (dep add), 946 (note); call sites 1510, 2350-area, 2362-area, 2384-area.
...
**Source:** 934–943. (The same scrape pattern recurs in `post_close_audit`'s regression-bead create, ~1248.)
...
**Source:** 910–922 (generic template + closing dep-add instruction); the overflow-specific reason is composed at the CONTEXT_OVERFLOW dispatch arm (2356–2370 region).
...
**Source:** 948–1021 (gate 956; bridge 997–1004; emit 1013–1015).
...
**Source:** 1049–1060 (`append_runner_note`), called from every failure branch + the max-retries gate.
...
**Source:** 1039–1045 (`record_incident`), 1065–1076 (`print_incidents_summary`), 330–331 (rotation excludes `incidents.log`), `INCIDENTS_LOG` 341; summary printed at 2307/2325/2427/2455.
...
**Source:** 1085–1125 (definition; patterns self-flagged brittle at ~1103), invoked at 2410 (outside the dispatch `case`).
...
**Source:** 1279–1288 (`notify_user`); noisy call sites 2303/2321/2350/2362/2397/2423 + 1115 (subagent) + 1272–1274 (discipline); silent: 2340 (rate-limit), 2381 (first task-not-closed), SUCCESS (none).
...
**Source:** 1889–1916 (`rate_limit_event` case; `RATE_LIMIT_QUOTA=` at 1910).
...
**Source:** 1029–1034 (`preserve_stream`), 2267–2274 (preservation class list), 2414–2416 (proc snapshot retention keyed on classification).
...
**Source:** 1730–1732 (per-iteration set), 2269 (preserve call with class suffix).
...
**Source:** 1457–1462; interacts with BC-19 and BC-58.
...
**Source:** 296 (startup rm), 1313–1318 (top-of-loop check), 1321–1335 (chunked sleep + `break 3` at 1330).
**Classification:** **SCAR.** The chunked-sleep + `break 3` is a direct responsiveness fix. **SCAFFOLDING:** `break 3` level-count is bash-loop-nesting mechanism.
...
**Source:** 492–559 (`daemon_ask_capacity`), 1533–1585 (call site + lease-release-on-deny), config `CAPACITY_DENY_BACKOFF` line 67.
...
**Assertion:** `runner_cleanup` (the project hook) is invoked from `cleanup()` (INT/TERM, line 389) and before the three fatal exits (AUTH 2304, BILLING 2322, breaker 2424). It is **not** invoked on the normal/graceful path (queue drained or stop file → `break` → fall off end at 2451–2477), and **not** on an unguarded `set -e` abort.
...
**Source:** 112 (default no-op), 389, 2304/2322/2424, 2451–2477 (no `runner_cleanup` on normal end).
...
**Correction vs the original "no EXIT trap" finding:** there **is** a `trap _final_subshell_reap EXIT` (line 414). So the *subshell*-leak gap the original entry flagged is now **closed** — `_final_subshell_reap` runs on every exit path including a `set -e` abort. What is *not* covered on a `set -e` abort is the `CURRENT_TASK_ID → open` reset (that lives only in `cleanup`, INT/TERM-only), so an abrupt `set -e` failure can still leave a task `in_progress` even though its subshells are reaped.

### BC-44 — Process-group isolation + EXIT-trap reap of TAIL/WATCHDOG/HB subshells
**Assertion:** Each per-task background subshell (the stream-parser `tail` pipe, the watchdog, the heartbeat) is spawned under `set -m` so the subshell leader's PID equals its PGID. They are reaped two ways: (1) the **per-task reap** after `wait`, in spawn-reverse order HB → WATCHDOG → TAIL, signals the **negative PID** (`kill -TERM -- -$PID` then, after a grace, `kill -KILL -- -$PID`, each with a pid-only fallback) so reparented `sleep`/`tail -f`/`jq` grandchildren are reached; (2) the **`trap _final_subshell_reap EXIT`** (line 414) PG-kills `WATCHDOG_PID`/`TAIL_PID`/`HB_PID` plus `pkill -P $$` on **every** exit path (clean drain, breaker exit 2, after `cleanup`'s exit 1, `set -e` abort), idempotently.
...
**Source:** 131–153 (`_final_subshell_reap`), 414 (EXIT trap), 1933/2052 (`set -m` spawn), 2104–2130 (per-task PG reap).
...
**Source:** 18–86 (defaults), 155–162 (config source), 267–275 (yolo), 374 (`runner_setup`), 1689–1693 (`PROMPT_EXTRA`).
...
**Source:** 164–218 (lib sourcing), 219–221 (`PROJECT_REF`).
...
**Source:** 39–49 (definition + rationale), 1773 (expansion at spawn).
...
**Source:** 1612–1661 (ASKBRIAN_BLOCK), 1671/1683 (inject), 1695–1725 (I4 resume splice).
...
**Source:** 257–265 (`lease_acquire_ok`/`lease_release_seam`); acquire/deny at the pickup (1527-area); releases at 387 / 1568-area / 2300-area / 2437-area.
...
**Source:** 850–893 (`detect_worker_stuck_primary`; gate 857–859; relaxed auto-flip 881–891); consumed 2161–2260, bypass 2281–2283.
...
**Source:** 1127–1275 (definition), 2294 (SUCCESS call site).
...
**Source:** 1306–1309 (override snapshot), 1545–1555 (per-iteration recompute).
...
**Assertion:** At task pickup (after lease+capacity pass, before the `in_progress` write) `CURRENT_TASK_ID` is set and **exported** so the `claude -p` worker and its hook subprocesses inherit it, and the id is also written to `$LOG_DIR/current-task` (best-effort) as a file fallback. The pointer is cleared at **startup** (a previous runner that died without clearing), at **loop-end** (2449), and on **interrupt** (404), so a between-tasks close-discipline hook can never enforce against a stale bead on the first tool call of the next task.
...
**Source:** 334–339 (startup clear), 1587–1596 (set/export/file), 404 (interrupt clear), 2449 (loop-end clear).
...
**Assertion:** A close-discipline hook is wired into the worker via `--settings <generated file>` (`HOOK_SETTINGS_FLAGS`) **only** when the close-checklist hook is executable AND `jq` is available; otherwise the flag array is empty and the worker runs without it. Two env vars are exported into the child at spawn: `BEADS_RUNNER_SESSION=1` and `POST_TERMINAL_FILE=<path>`.
**Repro:** Remove/`chmod -x` the close-checklist hook (or remove `jq`) → the worker spawns with no `--settings` and no close-hook enforcement, run otherwise unchanged.
**Source:** 1752–1762 (`HOOK_SETTINGS_FLAGS`), 1767–1778 (spawn + exported env).
**Classification:** **SCAR (intent) / SCAFFOLDING (mechanism).** The runtime enforcement of close-discipline is the intent (paired with BC-56's post-hoc audit); the `--settings`-file generation and the executable/`jq` precondition are mechanism.
...
**Source:** 234–240 (`hb`), 2038–2085 (mid-task HB subshell), config `HEARTBEAT_INTERVAL`/`HEARTBEAT_GAP_TOL` inlined at use.
...
**Source:** parser appends 1837–1916; watchdog appends `WATCHDOG_KILL=1`/`POST_TERMINAL_KILL=1` (1968/2014-area); join-then-classify 2089–2159.
...
**Source:** 1873 (write) vs. 798–818 (classifier never greps `RESULT_IS_ERROR`).
...
**Source:** throughout (representative: 264, 386, 401, 603, 614, 641, 680, 941, 1060, 2300, and every `command -v`-guarded seam).
...
**Source:** 1767–1778; defaults 20–49.
...
- **BC-21 / BC-36 — the "no EXIT trap" finding is now WRONG.** A `trap _final_subshell_reap EXIT` (line 414) runs the subshell PG-reap on every exit path (BC-44). The *subshell*-leak gap is closed; what remains asymmetric is `runner_cleanup` (INT/TERM + 3 fatal exits only) and the `CURRENT_TASK_ID→open` reset (INT/TERM only — a `set -e` abort still strands the task, though its subshells are now reaped).
...
- **BC-09 stale comment** at line 1156 (`post_close_audit`) still cites "line 743" for the fail-open echo, which is now line 790. Cosmetic only.
