...
> exists today (**2601 lines**, refreshed against the script at this commit). It
...
> since ~quadrupled to 2601 lines across ~40 commits (idle-on-empty-queue,
...
> ref against the current 2601-line script** — the +124-line net drift came from four
> commits landed after that refresh (per-commit insertion counts shown): `uxg8` (cross-WS scope check, +81 → BC-08d),
...
> `m0yv` (gate-wait discipline in the worker prompt, +5 → BC-38). All prior BC-IDs
> are preserved for referential stability; assertions were re-confirmed against the
> code (only BC-36's embedded numbers were stale, now corrected).
>
> Classification key:
...
**Source:** lines 1–3 (header intent); 1891–1902 (per-task `claude -p` spawn inside the main loop).
...
**Source:** 897–907 (exit-0 branch; fail-open stderr echo at 902); consumed at 2283.
...
**Source:** 914–930 (ordered greps in `classify_failure`); marker emission in the parser at 1943–2009 (esp. 1954–1966 api_retry, 1992–2009 result).
...
**Source:** 920–922 (ordering in classifier), 2006–2008 (parser pattern-match + `is_error` guard).
...
**Source:** 1965 (api_retry path), 1998 (result-stop-reason path), 922–924 (classifier collapse).
...
**Source:** 1618–1635 (top-of-loop `MAX_RETRIES` gate), 2457–2466 (RATE_LIMIT), 2468–2494 (overflow/max-tokens skip), 2496–2513 (task-not-closed retry-once), 2515–2527 (generic distinct-task breaker increment).
...
**Source:** 2515–2527 (distinct-task increment), 2412 (`SUCCESS` resets), 2542–2557 (breaker + rationale string), config default line 62.
...
**Source:** 2424, 2442, 2460, 2471, 2483, 2499, 2518 (uniform `--status=open`); STUCK bypass 2405–2406.
...
|------|-----------|--------|
| **0** | Normal end: queue drained with `RUNNER_EXIT_ON_DRAIN=1`, **or** graceful stop file consumed (`break`/`break 3`). Falls off the end of the script after the `while` loop. | 1429/1447/1525/1544/1551/1563/1612 (`break`), 2577–2601 (tail; no literal `exit 0`) |
| **1** | `SIGINT`/`SIGTERM` received → `cleanup()` trap. | 411–440 (`exit 1` 439) |
| **2** | Consecutive-failure circuit breaker tripped. | 2542–2557 (`exit 2` 2556) |
| **3** | `AUTH_FAILURE` (terminal). | 2421–2437 (`exit 3` 2436) |
| **4** | `BILLING_ERROR` (terminal). | 2439–2455 (`exit 4` 2454) |
...
**Source:** 2058–2157 (loop; threshold 2127, soft warning 2154, snapshot 2135–2141, staged kill 2146–2151).
...
**Source:** 2117–2127 (`EFFECTIVE_TIMEOUT` math); config 109/117; parser maintenance of `TASK_INFLIGHT_FILE` (BC-60).
...
**Source:** parser stamp 1934–1941; watchdog branch 2080–2097; config 127.
...
**Source:** `ACTIVITY_FILE` stamp 1912/1924; `TASK_INFLIGHT_FILE` 1911/1974–1987; epoch-floor guards 2107/2082/2198.
...
**Source:** 1013–1019 (label guard), 1037–1044 (create), 1053 (dep add), 1058 (note); call sites 1627, 2475-area, 2490-area, 2507-area.
...
**Source:** 1021–1034 (generic template + closing dep-add instruction); the overflow-specific reason is composed at the CONTEXT_OVERFLOW dispatch arm (2480–2491 region).
...
**Source:** 1151–1157 (`record_incident`), 1177–1188 (`print_incidents_summary`), 363–364 (rotation excludes `incidents.log`), `INCIDENTS_LOG` 374; summary printed at 2431/2449/2551/2579.
...
**Source:** 1197–1237 (definition; patterns self-flagged brittle at ~1215), invoked at 2534 (outside the dispatch `case`).
...
**Source:** 1391–1400 (`notify_user`); noisy call sites 2427/2445/2474/2486/2521/1626 + 1227 (subagent) + 1386 (discipline); silent: 2457 (rate-limit), 2496 (first task-not-closed), SUCCESS (none).
...
**Source:** 2013–2040 (`rate_limit_event` case; `RATE_LIMIT_QUOTA=` at 2034).
...
**Source:** 431–432 (interrupt), 2379–2380 (stuck), 2433–2434 (auth), 2451–2452 (billing), 2553–2554 (breaker), 2583–2584 (clean).
...
**Source:** 1141–1146 (`preserve_stream`), 2391–2398 (preservation class list), 2536–2540 (proc snapshot retention keyed on classification).
...
**Source:** 1853–1855 (per-iteration set), 2393 (preserve call with class suffix).
...
**Source:** 1574–1579; interacts with BC-19 and BC-58.
...
**Source:** 458–460 (sidecar globals init), 464–542 (`check_usage`; LA delegation 490–503 incl. reason/number capture 493/499–500, inline fallback 505–541 incl. capture 530/534/539), 1439 (loop-top pause line naming the gate), 1440 (force-fresh after sleep), 2465 (force-fresh after rate-limit).
...
**Assertion:** `runner_cleanup` (the project hook) is invoked from `cleanup()` (INT/TERM, line 422) and before the three fatal exits (AUTH 2428, BILLING 2446, breaker 2548). It is **not** invoked on the normal/graceful path (queue drained or stop file → `break` → fall off end at 2575–2601), and **not** on an unguarded `set -e` abort.
...
**Source:** 133 (default no-op), 422, 2428/2446/2548, 2575–2601 (no `runner_cleanup` on normal end).
...
**Source:** 152–174 (`_final_subshell_reap`), 447 (EXIT trap), 1920/2057/2176 (`set -m` spawn), 2215–2254 (per-task PG reap).
...
**Source:** 18–129 (defaults), 178–183 (config source), 290–296 (yolo), 407 (`runner_setup`), 1812–1817 (`PROMPT_EXTRA`).
...
**Source:** 39–49 (definition + rationale), 1897 (expansion at spawn).
...
**Source:** 1731–1780 (ASKBRIAN_BLOCK), 1790/1807 (inject), 1819–1849 (I4 resume splice).
...
**Source:** 278–286 (`lease_acquire_ok`/`lease_release_seam`); acquire/deny at the pickup (1644-area); releases at 420 (interrupt) / 1684 & 1699 (capacity-deny) / 2562 (per-task end) / 2432 & 2450 & 2552 (fatal-exit).
...
**Source:** 962–1005 (`detect_worker_stuck_primary`; gate 969–971; relaxed auto-flip 993–1003); consumed 2283–2384, bypass 2405–2407.
...
**Source:** 1251–1387 (definition), 2418 (SUCCESS call site).
...
**Source:** 367–372 (startup clear), 1706–1713 (set/export/file), 437 (interrupt clear), 2570–2573 (loop-end clear).
...
**Source:** 316–326 (optional `hooks/build-settings.sh` source + its degrade comment), 1877–1889 (`HOOK_SETTINGS_FLAGS` via `command -v build_hook_settings` guard + return-code gate; `build_hook_settings` call 1883–1884), 1891–1901 (spawn + exported env); the `--settings` JSON shape lives in `hooks/build-settings.sh`'s `build_hook_settings` (jq precondition `command -v jq || return 1` now inside it).
...
**Source:** 255–261 (`hb`), 2176–2209 (mid-task HB subshell), config `HEARTBEAT_INTERVAL`/`HEARTBEAT_GAP_TOL` inlined at use (2178–2179).
...
**Source:** parser appends 1961–2034; watchdog appends `WATCHDOG_KILL=1`/`POST_TERMINAL_KILL=1` (2129 / 2086); join-then-classify 2213–2283.
...
**Source:** 1997 (write) vs. 893–931 (classifier never greps `RESULT_IS_ERROR`).
...
**Source:** throughout (representative: 285, 419, 437, 695, 761, 798, 1058, 1382, 2424, 2562, and every `command -v`-guarded seam).
...
**Source:** 1891–1901; defaults 20–49.
...
### Re-verification pass — 2601-line script (claude-tools-c8c)

- **Every entry's `file:line` ref re-derived against the current 2601-line script.** The +124-line drift since the prior (2477-line) refresh came from four commits — `uxg8` (+81), `zfxe` (+34), `2fkp` (+26), `m0yv` (+5) — so nearly every ref at/below the `validate_task` tail (~line 712) shifted by +80…+124. Refs above that point (config block 20–108, defaults, `GUARDRAIL_FLAGS` 39–49) did not move.
