...
# the workspace_inventory producer (claude-tools-gk17 Phase A redo; epic
...
#      inventory-put.
#   9. Stage normalisation (the gk17 fix): bead with no labels, empty labels,
#      stage:impl, and mixed labels (other:x, stage:review) all produce a
#      non-empty stage string — the CF wire contract (reconcile.js:541-551)
#      rejects empty stage with 422, so this is the canary-failure guard the
#      ztb6 cut missed.
#
...
# increasing updated_at (so we can verify ordering / cap). Every bead gets
# stage:impl so the default fixtures exercise the happy-path stage extraction.
gen_open_beads() {
  local n="$1" i out="["
  for ((i=1; i<=n; i++)); do
    [[ "$i" -gt 1 ]] && out+=","
...
# ── Case 9: stage normalisation — the gk17 fix ───────────────────────────────
# The ztb6 cut (reverted) emitted stage:"" for any bead without a stage:* label,
# which the CF handler at reconcile.js:541-551 rejects with 422. Every entry
# in in_progress_beads[] and top_n_beads[] must carry a non-empty stage string.
# These cases pin the four shapes the producer must handle: no labels at all,
# empty labels array, single stage:impl label, mixed labels with stage:review.
echo "── Case 9: stage normalisation — empty/missing labels resolve to 'unknown' (gk17) ──"
: > "$OUTBOX"
rm -f "$WIP_BD_FIXTURE"/*.json
# 4 in_progress beads, one per case (a) no labels field, (b) empty labels
# array, (c) stage:impl, (d) other:x + stage:review. All must produce a
# non-empty stage; cases (a) and (b) must produce literally "unknown".
cat > "$WIP_BD_FIXTURE/list-in_progress.json" <<'JSON'
[
  {"id":"a","title":"no labels field","status":"in_progress","updated_at":"2026-05-01T00:00:00Z"},
  {"id":"b","title":"empty labels array","status":"in_progress","updated_at":"2026-05-02T00:00:00Z","labels":[]},
  {"id":"c","title":"stage:impl","status":"in_progress","updated_at":"2026-05-03T00:00:00Z","labels":["stage:impl"]},
  {"id":"d","title":"mixed labels","status":"in_progress","updated_at":"2026-05-04T00:00:00Z","labels":["other:x","stage:review"]}
]
JSON
# Same four beads, but in the open queue so we also cover top_n_beads.
...
# (a) no labels → "unknown"
ck "in_progress: bead with no labels field → stage='unknown'" \
   test "$(jq_field '.in_progress_beads[] | select(.bead_ref=="a") | .stage')" = "unknown"
ck "top_n: bead with no labels field → stage='unknown'" \
   test "$(jq_field '.top_n_beads[] | select(.bead_ref=="a") | .stage')" = "unknown"

# (b) empty labels → "unknown"
ck "in_progress: bead with empty labels array → stage='unknown'" \
   test "$(jq_field '.in_progress_beads[] | select(.bead_ref=="b") | .stage')" = "unknown"
ck "top_n: bead with empty labels array → stage='unknown'" \
   test "$(jq_field '.top_n_beads[] | select(.bead_ref=="b") | .stage')" = "unknown"

# (c) stage:impl → "impl"
ck "in_progress: stage:impl label → stage='impl'" \
   test "$(jq_field '.in_progress_beads[] | select(.bead_ref=="c") | .stage')" = "impl"
ck "top_n: stage:impl label → stage='impl'" \
   test "$(jq_field '.top_n_beads[] | select(.bead_ref=="c") | .stage')" = "impl"

# (d) mixed labels (other:x, stage:review) → "review"
ck "in_progress: mixed labels [other:x, stage:review] → stage='review'" \
   test "$(jq_field '.in_progress_beads[] | select(.bead_ref=="d") | .stage')" = "review"
ck "top_n: mixed labels [other:x, stage:review] → stage='review'" \
   test "$(jq_field '.top_n_beads[] | select(.bead_ref=="d") | .stage')" = "review"

# CF wire-contract acceptance gate: NO entry may have an empty stage. This is
# the assertion that would have caught the ztb6 canary failure pre-deploy.
ck "in_progress_beads: all entries have non-empty stage (CF wire contract)" \
   test "$(jq_field '[.in_progress_beads[].stage] | map(select(. == "")) | length')" = "0"
