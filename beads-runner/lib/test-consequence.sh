...
item_ar() { jq -cn --arg i "$1" --arg s "$2" --argjson cb "$(cb "$1" 1)" '
...
item_po() { jq -cn --arg i "$1" --arg s "$2" \
      --argjson a "$(cb "${1}A" 1)" --argjson b "$(cb "${1}B" 1)" '
...
item_ff() { jq -cn --arg i "$1" --arg s "$2" --argjson cb "$(cb "$1" 1)" '
...
item_rec() { jq -cn --arg i "$1" --arg s "$2" --argjson cb "$(cb "$1" 1)" '
...
item_obj() { jq -cn --arg i "$1" --arg s "$2" --argjson cb "$(cb "$1" 1)" '
...
do_dossier_put "$GOOD" "$(mk dA 1 "[$(item_ar i1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dC 1 "[$(item_ar c1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dP 1 "[$(item_po p1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dR 1 "[$(item_ar sib1 open),$(item_ff ff1 open),$(item_ar sib2 open)]")" >/dev/null
...
do_dossier_put "$GOOD" "$(mk dE 1 "[$(item_rec e1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dD 1 "[$(item_rec d1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dO 1 "[$(item_obj o1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk d15 1 "[$fifteen]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(jq -c '.items[0].consequence_block.cb_schema_version=2' \
   <<<"$(mk dV 1 "[$(item_ar v1 open)]")")" >/dev/null
: > "$BD_LOG"
ckn "cb_schema_version 2 ⇒ apply REJECTED (§0.3 unknown higher)" do_item_apply "$GOOD" dV v1 "$RESP_APPROVE"
...
# OMITTED from `bd create` (bd's default), NEVER synthesised by the applier.
do_dossier_put "$GOOD" "$(mk dB 1 "[$(item_bare b1 open)]")" >/dev/null
: > "$BD_LOG"
...
do_dossier_put "$GOOD" "$(mk dK 1 "[$(item_ar k1 open),$(item_ar k2 open)]")" >/dev/null
...
do_dossier_put "$GOOD" "$(mk dN 1 "[$(item_ar n1 open)]")" >/dev/null
...
do_dossier_put "$GOOD" "$(mk dX 1 "[$(item_ar x1 open)]")" >/dev/null
