...
post_close_audit() {
  ...
  if (( ${#failures[@]} == 0 )); then
    ...
    # bead's change set — a concurrent sibling / aux session / stray human edit)
    # is NOT a discipline bypass. Downgrade to a forensic incident only (option
    # C): NEVER a P1 regression bead and NEVER a DISCIPLINE_BYPASS marker note,
    # which would be a false accusation against a worker that closed clean.
