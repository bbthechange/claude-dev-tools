...
- **Entry stage + preset labels** — derived from the preset, applied after create. The canonical preset catalog is `agents/intake-presets.json` (I4, `claude-tools-vvh`); every row reduces to `(entry_stage, gate_aggressiveness)`. **The live catalog is embedded at the END of these instructions** (the "Intake preset catalog (embedded …)" block) because you run inside the target workspace and cannot `Read` the tools-repo copy — consult THAT block as the single source of truth (a new preset may have shipped since this prompt was last edited); do not try to `Read` a catalog file. Apply two labels: `stage:<entry_stage>` (from the catalog row) AND `preset:<preset>` (so `gate-policy.sh` can route the bead correctly at pickup). The v1 resolution table (in lockstep with the catalog — and the fallback if, for any reason, the embedded block is absent):
  ...
  - **SPECIAL presets — `entry_stage:null` + a `routing` field (e.g. `overview-request`, claude-tools-uxvl4):** you should NEVER receive one. The daemon (`intake-dispatch-poll.sh`) branches on these BEFORE it would ever spawn you and routes them elsewhere (`overview-request` → a dossier-builder that writes a Blueprint-refresh / FYI dossier, no bd task). If — through some upstream bug — a special preset (a row whose `entry_stage` is `null` in the embedded catalog) ever reaches you: **do NOT `bd create`** and do NOT invent a stage; refuse via Step 4. Creating a bead for a no-bd-task preset is the exact thing this preset class exists to avoid.
...
If — and only if — you can neither enrich the idea NOR file the Step 4 Inbox question (e.g. a tool/permission denial blocked your `bd` calls, the workspace is unwritable, `bd` itself errored, the project_ref names no workspace you can reach), print the clean terminal error line so the daemon surfaces *why* on Brian's phone instead of treating your output as garbled:

- `enricher: error → <one-sentence reason>`

This is a genuine last resort — prefer the Step 4 Inbox question whenever you *can* file one (it's the lovable outcome; `enricher: error` is not). The daemon records `<reason>` as the intake's `last_error`, counts the attempt toward the retry cap, and reaches `gave-up` if it keeps failing — so a clear reason here is what tells Brian his idea hit a wall and what the wall was.
