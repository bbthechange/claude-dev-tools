...
4. **No internal jargon in user-facing text — the readability gate (inbox-lifecycle §4.4).** Brian's reading this on a phone. The bar: a non-author reading this dossier **cold** must learn, in **under 30 seconds and without any contract jargon**, (a) **what blew up**, (b) **what they're being asked to decide**, and (c) **what each option actually does**. So: don't reference internal contract section numbers, don't write `AD7` or `BC-34` or `§5.2`, don't surface raw state-machine / reason tokens (`worker_stuck`, `blocked-for-human`, `STUCK_NEEDS_HUMAN`), don't say "per the freeze." **Never name an internal section, ID, or state without translating what it means in plain English in the SAME sentence** — and better still, don't name it at all; say the plain-English thing it stands for. Example: ❌ "the worker slipped past the §7.6 guardrail" → ✅ "the worker isn't allowed to ask interactive questions, so it couldn't pause and ask you in-band — it stopped instead." Expand acronyms on first use. Explain the **why** before the **how**. Plain English; that's the contract. (This is checkable: `dg__readability_lint` in `beads-runner/lib/dossier-gen.sh` flags untranslated jargon tokens — section symbols, contract IDs, raw enum/state tokens — in the reader-facing prose; the deterministic fallback template is held to the same lint. It's advisory, not a write gate, but a dossier that trips it is one Brian can't read.)
...
| `context_anchor` | yes — mandatory, no exceptions | `{ where: string, expansion: string, link?: string }`. **Self-contained-context invariant.** `where` says where this item sits in the lifecycle / design / bead graph; `expansion` is enough prose that the question is answerable without going hunting. If you can't author a real one, **refuse the dossier** (see Step 4). **`link` — the Blueprint bridge (§6.4, claude-tools-uxg3):** when the decision is *structural* (touches a domain/component that lives on the workspace Blueprint), set `link` to the Blueprint focus deep-link `/ws/<ref>/blueprint?focus=<node-id>` (relative path; `<ref>` = the workspace bd-prefix, `<node-id>` = the Blueprint §4 stable node id for that domain/component, which you can read from the workspace's blueprint record). The Inbox renders it as a "where this sits on the map" affordance so the decision is *situated* — the dossier borrows a focus-view, it never duplicates the map. Optional and best-effort: omit `link` when the matter is non-structural or you can't confidently name the node (a non-blueprint or non-`http(s)` URL simply mints no bridge — nothing extra renders). |
