...
Status: draft (v2 — adversarial review integrated) · Scope: **architecture/design only** (deliberately *not* UX) · Owner: Brian · Last updated: 2026-05-15
...
|---|---|---|
| **AD1** | **Topology = Cloudflare hosted Coordinator + a per-computer Local Agent (§2).** Coordinator = Workers + Durable Objects + D1 + Pages. | $0 at this workload; a **DO-per-Decision** makes the consequence applier idempotent *by construction* (single-threaded) and `setAlarm()` is the `timed-fyi` timer. **SPOF acknowledged:** the singleton Coordinator DO is a single point of failure on free-tier infra with non-contractual alarm reliability; mitigated by the C-2 unreachable posture and the S-6 timer backstop, not waved away. |
...
| **AD3.4** | **Double-trigger idempotency:** worker-self-signal + backstop on the same fork de-dupe via the DO-per-Decision keyed on task ID (AD1 makes blocking+`bd human`+dossier idempotent). | One fork ⇒ one dossier. |
...
| C5 | 4-pass dossier orchestration | 0.C/A·B | **SIMPLIFY** | One structured generation, frozen output schema. |
...
**Seam:** freeze the versioned output schema
`{filtered_context, framing, consequence_block, response_affordances}`;
producer swappable behind it. **v1 MUST** version that schema; downstream binds
to schema, never "the passes."
...
1. small strongly-consistent store (Decision / RunnerState{desired,actual,**liveness**} / Notification / lease / work-snapshot),
2. durable one-shot timer (`fire(decisionId) at T`) **+ S-6 backstop** (a missed alarm degrades to fire-on-next-poll; the idempotent DO-per-Decision dedups alarm-fire vs poll-fallback so `timed-fyi` is *eventually* applied exactly once even if the free-tier alarm is unreliable — never an infinite stall),
