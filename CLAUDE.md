...
| `docs/context/web-facets.md` | The UX-v2 facet routes: the per-workspace hub, the workspaces list, the capacity view, the cross-workspace view. Code: `beads-runner/web/{workspace,workspaces,capacity,cross-ws}/`. (Blueprint **diagram** internals → its own doc ↓.) |
| `docs/context/web-blueprint.md` | The Blueprint **diagram** (Flow H): the pan/zoom system map — pure read-model + write-model, the left→right band layout, the edge-resolution/density IP, the APIs/Internals/edge-labels/in-flight toggle panel, minimap, drill-in/focus, palette, customization, and the §17/bfcache scars. Code: `beads-runner/web/workspace/blueprint-view.js` + `blueprint-customize.js` + the `renderBlueprint*` block of `app.js`. |
