...
- **Missing `mcp__askbrian__ask-brian` in the allowlist** → agents see the tool exists (user-scope MCP makes it visible everywhere) and try to call it, hit permission denial, fall through to the bd-notes fallback. Beads park as `status=blocked label=human`, no dossier on your phone. Look in `bd list --status=blocked --label human` for stranded work.
