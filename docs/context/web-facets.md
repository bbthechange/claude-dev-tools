...
- `web/workspace/` — the per-workspace **hub / facet shell** (`/ws/<ref>/<facet>`).
  The `board`, `activity` **and `gates`** facets are real; `blueprint` is still an
  honest placeholder *mount* (H3) but its pure **map renderer now exists**
  (`blueprint-view.js`, H2 (claude-tools-uxvh2)). (`activity` graduated in I3
  (claude-tools-uxvi3); `gates` graduated in J3 (claude-tools-uxvj3).)
...
| `workspace/` (blueprint) | **Renderer real (+ in-flight overlay), mount pending** (H2 shipped `blueprint-view.js`; G5 (claude-tools-uxg5) added the §6.4/§8.2 in-flight overlay — `opts.active_domains`/`opts.activity` light up worked domains + the two-agents-one-domain collision flag. The facet still `mountPlaceholder`s — H3 mounts the map + narrative + the live `blueprint-get` fetch + the overlay wiring + `?focus` deep-links). |
