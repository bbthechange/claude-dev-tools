...
| `workspace/` (blueprint) | **Renderer real, mount pending** (H2 shipped `blueprint-view.js`; the facet still `mountPlaceholder`s — H3 mounts the map + narrative + the live `blueprint-get` fetch + `?focus` deep-links). |
