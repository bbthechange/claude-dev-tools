...
| `src/agent-action.js` | I4 (claude-tools-uxvi4, design/agent-action.md §2) the control-plane `agent_actions` **transient** command queue: `agent-action` (enqueue — closed host-effecting intent enum {nudge,kill-retry,kill-gate,gate-apply,gate-lift,**set-desired**} + per-intent required fields, `owner` an INPUT §2.4, `crypto.randomUUID()` action_id, runs INSIDE `co._serialize`) + `agent-action-pending` (daemon read, optionally one `workspace`; pure read) + `agent-action-ack` (daemon write — terminal done\|failed). NOT a §4 record. Adapter-maps ONLY `agent-action` (the web write path; `-pending`/`-ack` are daemon-native via `POST / {op,args}`). Bash-oracle twin IS present (`lib/coordinator.sh` `co__agent_action_*` — closed the gate-meta gap). Migration `0011_agent_actions.sql`. The daemon EXECUTOR is `daemon/agent-action-poll.sh` (drops the runner control marker for the process intents; runs `gate-defer.sh` for the gate intents; **for `set-desired` writes the LOCAL `.co-store/runner_state.desired` — the local-first change-request consumer, claude-tools-y6j9**). **`set-desired`** (claude-tools-y6j9 — local-first desired-state) carries the envelope-level `workspace` (validated by `safeKey`, like every intent; `target` is `{}`) + `args.state ∈ {running,paused,spare-cycles,stopped}` (the FIRST place the desired enum is validated — `opSetDesired`/`co__set_desired` never did); it is the agent_actions INTENT, distinct from the legacy `set-desired` OP (`opSetDesired`, the direct RunnerState.desired write the GUI keeps for display). The cf change is the two-line enum+validation widening; no DDL/adapter change (the generic `target_json`/`args_json` columns + the existing `agent-action` adapter mapping carry it). Escalate is NOT here — it's a `dossier-generate` write (`web/functions/api/control/escalate.js`). |
