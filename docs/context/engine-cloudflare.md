...
| `src/reconcile.js` | **`workSnapshot()`** — the read-only `§4.5` projection producer (Contract B). Joins `runner_state` + work-truth + `machine_state_reports` + capacity. The board/inbox read-model. S-1 liveness derived at read time, never stored. |
...
| `src/timer.js` | §2.2 `fire(dossier_id)` timer wiring + the S-6 fire-on-next-poll backstop + timed-fyi auto-proceed. |
