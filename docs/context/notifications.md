...
| `web/inbox/sw.js` | The service worker. Three handlers — `push` (render triage notification), `notificationclick` (deep-link), and `pushsubscriptionchange` (claude-tools-qlj0: re-POST a browser-rotated subscription to `/api/push/subscribe` so delivery self-heals instead of silently dying on a pruned stale endpoint). The last needs the public VAPID key + `urlB64ToUint8Array` — both DUPLICATED from `push.js` (no shared scope; rotate the two copies in lockstep). **Still no fetch/offline cache** (the Inbox must never serve a stale projection, S-1). |
