...
| `attention-poll.sh` | **iz36** (claude-tools-iz36) + **tv88** (claude-tools-tv88): the MACHINE-ATTENTION poll. Reads the engine's top-level `work-snapshot.attention` field (singleton call, the N2 workspace[0] pattern; ~60s + boot-fire) and emits an EDGE-triggered structured `WARN MACHINE ATTENTION workspace=… kind=stale-runner …` per sustained wanted-running-but-wedged runner — the observability half of the jzzw incident. Markers in `$DAEMON_CACHE_DIR/attention-fired/` dedup (no 60s storm), re-log at most hourly while ongoing (`DAEMON_ATTENTION_RELOG_SECONDS`, 0 disables), and `RESOLVED` + remove on clear. Observe-first: an empty/unreachable snapshot does NOT flap RESOLVED. Off-switch `DAEMON_ATTENTION_DISABLED=1`. **tv88 added the ACTIVE PHONE PUSH half:** on the EDGE `_daemon_attention_push` ALSO shapes a kind=overview / **tier=digest** §4.1 dossier (DETERMINISTIC body — the stale-runner remediation is fixed/operational, NOT a dossier-builder spawn; the §5.1 write gate still runs at dg_generate so a malformed body is rejected) and emits a digest §4.3 notification on the SHARED `machine-attention` channel via `dg_generate`+`no_emit`+`no_dispatch` (the `_daemon_flow_f_engine_write` SEQUENCE, digest-flavored, **NO tf_arm** — digest≡no-armed-timer). The shared channel folds N alerts → **ONE** daily push (must-protect #5). One push per sustained alert: a `<marker>.pushed` sentinel dedups + makes a failed-edge push retry next cadence (the blueprint-update fyi-pending discipline). **CANARY-DISABLED by default** (`DAEMON_ATTENTION_PUSH_DISABLED=1`) — offline-green but DORMANT until the §2.5 device live-verify (claude-tools-yhp8 owns the flip+verify). Defines `daemon_attention_poll_once`, `_daemon_attention_build_gi`, `_daemon_attention_engine_write`, `_daemon_attention_push`. |
...
update: 2026-06-15 (tv88: `attention-poll.sh` grew the ACTIVE PHONE-PUSH half —
on the EDGE it shapes a tier=digest overview dossier + emits a digest §4.3
notification on the shared `machine-attention` channel via the
`_daemon_flow_f_engine_write` sequence minus `tf_arm`; DETERMINISTIC body, N→1
fold (must-protect #5), `.pushed` retry-sentinel, CANARY-DISABLED until the
claude-tools-yhp8 device-verify). Prior: 2026-06-14 (iz36: `attention-poll.sh` —
the machine-attention observability poll reads the engine `work-snapshot.attention`
field and edge-WARNs each sustained wanted-running-but-wedged runner; the jzzw
incident follow-up #3). Prior:
