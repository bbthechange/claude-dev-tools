...
| `timed-fyi.sh` | §2.2 `fire(dossier_id)@T` timer for the timed-fyi auto-proceed window + the S-6 missed-fire ⇒ fire-on-next-poll backstop (`tf_fire` shared by alarm + poll-fallback). See `notifications.md`. |
